    ...
    <a-card>
      ...
      >
        <a-form-model-item class="a" label="产品" prop="pid">
          <a-select v-model="source.pname" placeholder="选择产品">
             <a-select-option v-for="(it,i) in prouct" :key="i" >{{it.pname}}</a-select-option>
          </a-select>
        </a-form-model-item>
        ...
        >
          <a-date-picker
            v-model="source.endtime"
            show-time
            format="YYYY-MM-DD"
            value-format="YYYY-MM-DD"
          />
        </a-form-model-item>
        <a-form-model-item label="进货类型" class="a" prop="type" v-if="false">
          <a-radio-group v-model="source.type" :default-value="0">
...
<script>
import { update,getproduct } from "@/services/stockIn";
...
export default {
  ...
  methods: {
    Insubmit(source) {
      this.source.pid=this.prouct[this.source.pname].pid
      this.$refs[source].validate((valid) => {
        if (valid) {
          // 如果校验通过，请求接口，允许提交表单
          update(this.source).then(this.afterSubmit);
  ...
  created() {
    ...
    this.source.type = Number(this.$route.query.data.type);
    getproduct().then(res=>{
      if(res.data.msg!=""){
                this.$message.success(
            // '查询成功',
    ...
    //this.source.producetime= new Date(this.source.producetime.setDate(this.source.producetime.getDate() + 1))
    //this.source.endtime=  new Date(this.source.producetime.setDate(this.source.producetime.getDate() + 1))
    //alert(this.source.producetime)
    //alert( this.source.type)
  },
  watch: {
    $route(to, from) {
      console.log(to.path);
